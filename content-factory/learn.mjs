...
import { rpush, lrange, kvGet, kvSet } from "./store.mjs";
...
// ---- 기록 ----
export async function recordReview(stat, issues) {
  try {
...
export async function refreshLessons() {
  ...
  const lessons = (
    await ask(
      `너는 매거진 편집장이다. 아래는 AI 에디터가 최근 반복적으로 지적받은 사항들이다.
