...
export async function runAudit(opts = {}) {
  ...
  try {
    ...
    out.errors.push(`노션 동기화: ${e.message}`);
  }
  ...
  try {
    ...
    out.errors.push(`발행글 로드: ${e.message}`);
  }
  ...
  for (const post of queue) {
    try {
      ...
      out.errors.push(`재감사 ${post.slug}: ${e.message}`);
    }
  }
  ...
  for (const post of rescuable) {
    try {
      ...
      out.errors.push(`구조 ${post.slug}: ${e.message}`);
    }
  }
  ...
  if (notionEnabled && 홈페이지읽힘) {
    try {
      const { getPostBySlug } = await import("./posts.mjs");
      const { listNotionByStatus } = await import("./notion.mjs");
      ...
      out.errors.push(`노션 대조: ${e.message}`);
    }
  }
  ...
  try {
    ...
    out.errors.push(`교훈 갱신: ${e.message}`);
  }
