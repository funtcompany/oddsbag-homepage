...
//   0    ~ 280 : 좌 카테고리+제목(최대 3줄, 절대 안 자름) / 우상단 ODDSBAG 로고
//   280  ~1630 : 카드뉴스 원본 1080×1350 그대로
//   1656 ~1712 : 글 특화 키워드 5개
//   1782 ~1824 : 중앙 하단 oddsbag.co.kr
//
...
export function fitTitle(title, avail = 760, maxH = TITLE_MAX_H) {
...
function frameEl({ post, cardDataUri, kws }) {
  ...
  const kids = [];
  // 우상단 로고
  kids.push(el("div", { display: "flex", alignItems: "center", position: "absolute", top: 54, right: 60 }, [
  ...
  // 좌측 카테고리 + 제목(영상 내내 고정)
  kids.push(el("div", { display: "flex", flexDirection: "column", position: "absolute", top: TITLE_TOP, left: 60, width: 760 }, [
  ...
  kids.push(el("div", { display: "flex", justifyContent: "center", position: "absolute", bottom: LINK_BOTTOM, left: 0, width: W }, [
    el("div", { display: "flex", fontSize: 34, fontWeight: 800, color: p.sub, letterSpacing: 0.5 }, "oddsbag.co.kr"),
  ]));
...
export function checkLayout(post) {
  ...
  if (KW_TOP + 56 > H - LINK_BOTTOM - 42) problems.push("키워드가 주소와 겹침");
