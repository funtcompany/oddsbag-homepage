...
import { getPublishedRaw, getDrafts, upsertPublished, type Post } from "@/lib/posts";
import { getQualityTrend, getLessons } from "@/lib/learn";
import { staleGuides, GUIDE_STALE_DAYS } from "@/lib/guideAge";
...
export interface ImproveResult {
  ...
  reshared: number;
  staleCount: number;
  stale: { slug: string; title: string; days: number }[];
  byCategory: Record<string, number>;
  trend: { avg7: number; avgPrev: number; autoPublishRate: number; count: number };
  actions: string[];
  errors: string[];
}
...
export async function runImprove(): Promise<ImproveResult> {
  ...
  try {
    ...
    out.errors.push(`로드: ${(e as Error).message}`);
  }
  ...
  for (const post of noCover.slice(0, FIX_COVERS_PER_RUN)) {
    try {
      ...
      out.errors.push(`커버 ${post.slug}: ${(e as Error).message}`);
    }
  }
  ...
  if (socialEnabled) {
    ...
    for (const post of missing) {
      try {
        ...
        out.errors.push(`재게시 ${post.slug}: ${(e as Error).message}`);
      }
    }
  }
  ...
  if (emailEnabled) {
    try {
      ...
      out.errors.push(`리포트 메일: ${(e as Error).message}`);
    }
  }
...
async function suggestActions(ctx: {
  ...
  staleCount: number;
  byCategory: Record<string, number>;
  trend: { avg7: number; avgPrev: number; autoPublishRate: number };
  lessons: string;
  subs: number;
  recentTitles: string[];
}): Promise<string[]> {
...
function reportHtml(r: ImproveResult, subs: number): string {
