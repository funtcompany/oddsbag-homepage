...
import type { DraftDraft } from "@/lib/ai";
...
const PASS_SCORE = 80; // 이 이상 + 위험 낮음 → 즉시 발행
const HOLD_SCORE = 62; // 이 미만 → 바로 검수함 (개선 시도 안 함)
...
출력은 반드시 아래 JSON만. 다른 말 금지.
{"scores":{"accuracy":0,"readability":0,"tone":0,"useful":0,"title":0},"fakeRisk":"low|medium|high","issues":["..."],"note":"한 줄 심사평"}`;
...
위 초안을 원문과 대조해 심사하라. JSON만 출력.`;

  const raw = await claude(REVIEW_SYSTEM, user);
  const p = parseJson<{
  ...
  const sc = p.scores ?? { accuracy: 0, readability: 0, tone: 0, useful: 0, title: 0 };
  const score = Math.max(
    0,
    Math.min(100, (sc.accuracy | 0) + (sc.readability | 0) + (sc.tone | 0) + (sc.useful | 0) + (sc.title | 0)),
  );
  const fakeRisk: FakeRisk = ["low", "medium", "high"].includes(p.fakeRisk) ? p.fakeRisk : "high";
  const issues = Array.isArray(p.issues) ? p.issues.slice(0, 6) : [];

  // 판정: 가짜뉴스 위험이 조금이라도 있으면 절대 자동 발행하지 않는다
  let verdict: Verdict;
  if (fakeRisk === "high") verdict = "hold";
  else if (fakeRisk === "medium") verdict = score >= PASS_SCORE ? "revise" : "hold";
  else if (score >= PASS_SCORE) verdict = "publish";
  else if (score >= HOLD_SCORE) verdict = "revise";
  else verdict = "hold";

  return { score, fakeRisk, verdict, issues, note: p.note ?? "", scores: sc };
}
...
출력은 반드시 JSON만: {"title":"...","summary":"...","body":"## ...","hook":"인스타 훅 한 줄"}`;
...
지적사항을 반영해 다시 써라. JSON만 출력.`;

  const raw = await claude(REVISE_SYSTEM, user, 1800);
  const p = parseJson<{ title: string; summary: string; body: string; hook?: string }>(raw);
  return {
    title: p.title?.trim() || draft.title,
    summary: p.summary?.trim() || draft.summary,
    body: p.body?.trim() || draft.body,
    hook: p.hook?.trim(),
  };
}
...
문제가 없으면 issues는 빈 배열, fakeRisk는 low, 점수는 높게 준다. 억지로 흠집 내지 마라.
...
이 발행글을 재감사하라. JSON만 출력.`;

  const raw = await claude(AUDIT_SYSTEM, user);
  const p = parseJson<{
    scores: Review["scores"];
    fakeRisk: FakeRisk;
    issues: string[];
  ...
  const fakeRisk: FakeRisk = ["low", "medium", "high"].includes(p.fakeRisk) ? p.fakeRisk : "medium";
  const issues = Array.isArray(p.issues) ? p.issues.slice(0, 6) : [];
...
지적사항만 정확히 반영해 다시 써라. 없는 사실을 새로 만들지 마라 — 위험한 문장은 추가하지 말고 삭제하라. JSON만 출력.`;

  const raw = await claude(REVISE_SYSTEM, user, 1800);
  const p = parseJson<{ title: string; summary: string; body: string; hook?: string }>(raw);
  return {
    title: p.title?.trim() || post.title,
    summary: p.summary?.trim() || post.summary,
    body: p.body?.trim() || post.body,
    hook: p.hook?.trim(),
  };
}
